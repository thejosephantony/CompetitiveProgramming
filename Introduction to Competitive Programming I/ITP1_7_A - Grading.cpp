...
int main(){
	...
	while(cont <s 50){
		cin >> exameInt >> exameFin >> exameRec;
		if((exameInt == -1) and (exameFin == -1) and (exameRec == -1)){
			break;
			return 0;
		}
		
		if((exameInt == -1) or (exameFin == -1)){
			cout << "F" << endl;
		}
		else if(exameInt + exameFin >= 80){
			cout << "A" << endl;
		}
		else if(exameInt + exameFin >= 65){
			cout << "B" << endl;
		}
		else if(exameInt + exameFin >= 50){
			cout << "C" << endl;
		}
		else if(exameInt + exameFin >= 30){
			if(exameRec >= 50){
				cout << "C" << endl;
			}
			else{
